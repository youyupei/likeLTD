...
optimisation.params.peaks <- function(hypothesis, verbose=TRUE, fixed=NULL,
  ...
  if(!is.null(fixed)) {
    ...
    linkBool = doLinkage&!identical(!hypothesis$relationship%in%c(0,1))&hypothesis$hypothesis=="prosecution"
  if(linkBool)
    {
...
evaluate.peaks <- function(P.pars, D.pars, tolerance=1e-6, n.steps=NULL, interim=FALSE, CR.start=0.1, CR.end=0.7, seed.input=NULL, nConverged=5){
