...
dropDeg <- function(hypothesis,results,args) 
	{
...
output.report <- function(admin,prosecutionHypothesis,defenseHypothesis, prosecutionResults,defenseResults,args,stain) {
  #
  # Args:
  #   admin: List containing administration data, as packed by
  #          pack.admin.input.
  ...
  # Match probability
  ideal.match=ideal(defenseHypothesis)
  ...
  # Hp dropout
  pDropout = dropDeg(prosecutionHypothesis,prosecutionResults,args) 
  textplot(pDropout,valign='top',cex=size)
  title('HP Drop-out',cex=hsize)
  # Hd dropout
  dDropout = dropDeg(defenseHypothesis,defenseResults,args) 
  ...
  NU HP:		number of unknown contributors under Hd.
  NU HD:		number of unknown contributors under Hp.
