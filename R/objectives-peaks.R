...
create.likelihood.vectors.peaks <- function(hypothesis, addAttr=FALSE, likeMatrix=FALSE, ...) {
  ...
  likelihood.vectors <- function(degradation=NULL, DNAcont=NULL, scale=NULL, stutter=NULL, dropin=NULL,
                                 repAdjust=NULL, degradationPenalty=hypothesis$degradationPenalty, ...) {
    ...
    if(length(stutter) == 1) stutter = rep(stutter, length(functions))
...
create.likelihood.per.locus.peaks <- function(hypothesis, addAttr=FALSE, likeMatrix = FALSE) {
  ...
  result.function <- function(scale,stutter,repAdjust=NULL,
                              degradation=NULL, DNAcont=NULL, dropin=NULL, ...) {
...
peaks.probabilities = function(hypothesis,cons,DNAcont,scale,stutter,degradation,repAdjust,doR=FALSE)#,doC=TRUE)
    {

repADJUST.PEAKS <<- repAdjust

PEAKS.PEAKS<<-hypothesis$peaksProfile
HEIGHT.PEAKS<<-hypothesis$heightsProfile
SIZES.PEAKS<<-hypothesis$sizesProfile
DNACONT.PEAKS<<-DNAcont
STUTTER.PEAKS<<-stutter
SCALE.PEAKS<<-scale
DEG.PEAKS<<-degradation
FRAG.PEAKS<<-hypothesis$alleleDb[,2]
...
GENOTYPES.PEAKS<<-genotypes
	#if(doC==FALSE)
	#	{
        	# probabilities for each replicate
        	probs = sapply(1:length(hypothesis$peaksProfile), FUN=function(x) peak.heights.per.locus(genotypes,hypothesis$peaksProfile[[x]],hypothesis$heightsProfile[[x]],hypothesis$sizesProfile[[x]],DNAcont,stutter,scale=scale,degradation=degradation,fragLengths=hypothesis$alleleDb[,2],repAdjust=repAdjust[x]))
	#	} else {
...
peak.heights.per.locus = function(genotypeArray,alleles,heights,sizes,DNAcont,stutter,scale,degradation,fragLengths,repAdjust,parallel=TRUE)
	{
	#index = !is.na(alleles)
	...
	if(parallel==FALSE)
		{
	        Probs = apply(genotypeArray,MARGIN=2,FUN=function(x) probability.peaks(x,alleles,heights,sizes,DNAcont,stutter,scale,degradation,fragLengths,repAdjust))
		} else {
	        Probs = mclapply(1:ncol(genotypeArray),FUN=function(x) probability.peaks(genotypeArray[,x],alleles,heights,sizes,DNAcont,stutter,scale,degradation,fragLengths,repAdjust),mc.cores=cores)
		}
	Probs = unlist(Probs)
	Probs[is.na(Probs)] = 0
	return(unlist(Probs))
...
# rhoA = allelic constant parameter, single value
# rhoS = stutter constant parameter, single value
probability.peaks = function(genotype,alleles,heights,sizes,DNAcont,stutter,scale,degradation,fragLengths,repAdjust,doC=TRUE)
	{
	...
	SCALE.SUB <<- scale
	DEG.SUB <<- degradation
	FRAGLENGTHS.SUB <<- fragLengths
	REPADJUST.SUB <<- repAdjust
...
PEAKHEIGHTS <<- peakHeights
	...
	S <<- scale
	X <<- peakHeights
	SHAPES <<- shapesVec+0.001
	SCALES <<- scalesVec+0.001
...
peak.height.dose = function(genotype,alleles,heights,sizes,DNAcont,stutter,degradation,fragLengths,repAdjust)
	{
...
FRAGLENGTHS <<- fragLengths[fragLengthIndex]
