...
optimisation.params <- function(hypothesis, verbose=TRUE, fixed=NULL,
 ...
 # population size for optimisation
  ...
  searchPop = round(searchPop * relFactor(hypothesis$relatedness))
	...
	PARAMS <- list(fn = result.function, 
		lower = unlist(lower), 
		upper = unlist(upper),
...
DEoptimLoop <- function(PARAMS, tolerance=1e-6)
	...
	# Note, optimisation.params() actually returns a list of 2 objects, one of which is PARAMS, hence:
	upstreamHypothesis <- PARAMS$upstreamHypothesis
	...
	results$upstreamHypothesis = upstreamHypothesis
	return(results)
	}
