...
<<optim, eval=FALSE, tidy=FALSE>>=
  params = optimization.params(defenseHyp, verbose=FALSE, throwError=FALSE)
  results <- do.call(optim, params)
  arguments <- relistArguments(results$par, defenseHyp)
@
...
\begin{tabular}{l|c}
  ...
  tvedebrink  & \Sexpr{format(arguments$tvedebrink, digits=3)}\\\hline
\end{tabular}
