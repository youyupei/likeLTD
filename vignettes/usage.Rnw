...
<<plotscalarWoE, tidy=FALSE, eval=FALSE>>=
  ...
  scalarWoE <- function(x) {
     defenseModel(locusAdjustment=list(D3=0.983, vWA=1.010,
                                       D16=1.028, D2=1.072,
                                       D8=1.020, D21=0.930,
                                       D18=0.850, D19=0.932, 
                                       TH01=1.041, FGA=0.916),
...
<<maxiskel, eval=FALSE, tidy=FALSE>>=
  ...
  upper = list(locusAdjustment = rep(Inf, nloci),
               dropout         = c(1, 1),
               degradation     = rep(Inf, 3),
               rcont           = rep(Inf, 2),
               dropin          = Inf,
               power      = -1e-8 )[names(arguments)]
  lower = list(locusAdjustment = rep(1e-8, nloci),
               dropout         = c(1e-8, 1e-8),
               degradation     = rep(-Inf, 3),
               rcont           = rep(1e-8, 2),
               dropin          = 1e-8,
               power      = -Inf )[names(arguments)]
...
<<optim, eval=FALSE, tidy=FALSE>>=
  ...
  params$control$factr=1e12 # Less strict convergence, for demo purposes.
...
\begin{tabular}{ccccc}
  ...
  \Sexpr{paste0(format(arguments$locusAdjustment[1:5], digits=3), collapse=" & ")}\\\hline
  \Sexpr{paste0(colnames(defenseHyp$cspProfile)[6:10], collapse=" & ")}\\
  \Sexpr{paste0(format(arguments$locusAdjustment[6:10], digits=3), collapse=" & ")}\\\hline
\end{tabular}\\[2ex]
