...
output.names <- function(caseName,stain,args,prosecutionHypothesis,caseFolder=caseName)
	{
...
output.report <- function(admin,prosecutionHypothesis,defenseHypothesis, prosecutionResults,defenseResults) {
  #
  # Args:
  #   admin: List containing administration data, as packed by pack.admin.input.
  #   prosecutionHypothesis: generated by prosecution.hypothesis()
  #   defenseHypothesis: generated by defense.hypothesis()
  ...
  defenseLikes <- locus.likes(defenseHypothesis,defenseResults)
  ...
  # Hd dropout
  dDropout = dropDeg(defenseHypothesis,defenseResults,admin) 
  ...
  # Dropin
  if(defenseHypothesis$doDropin==TRUE)
  	{
  ...
  # Match probability
  ideal.match=ideal(defenseHypothesis,defenseHypothesis$relatedness)
  ...
  administration = data.frame(
	...
	REFname,
	ALLELEname,
	paste(prosecutionHypothesis$ethnic,defenseHypothesis$ethnic),
	#initiated,
	endTime,
	paste(prosecutionHypothesis$adj,defenseHypothesis$adj),
	paste(prosecutionHypothesis$fst,defenseHypothesis$fst),
	paste(prosecutionHypothesis$relatedness[1],defenseHypothesis$relatedness[1]),
	paste(prosecutionHypothesis$relatedness[2],defenseHypothesis$relatedness[2]),
	Nknd,
	Nkdo,
	paste(prosecutionHypothesis$nUnknowns,defenseHypothesis$nUnknowns),
	paste(prosecutionHypothesis$doDropin,defenseHypothesis$doDropin),
	...
	paste(format(round(defenseResults$par[grep("rcont",names(defenseResults$par))],2),nsmall=4),collapse=' '),
