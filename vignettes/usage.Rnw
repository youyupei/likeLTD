...
\texttt{defenceModel} to perform an optimization or to create
...
<<optim, eval=FALSE, tidy=FALSE>>=
  params = optimization.params(defenceHyp, verbose=FALSE, throwError=FALSE)
  params$control$factr=1e12 # Less strict convergence, for demo purposes.
  results <- do.call(optim, params)
  arguments <- relistArguments(results$par, defenceHyp)
@
...
\begin{tabular}{ccccc}
  ...
  \Sexpr{paste0(colnames(defenceHyp$cspProfile)[1:5], collapse=" & ")}\\
  \Sexpr{paste0(format(arguments$locusAdjustment[1:5], digits=3), collapse=" & ")}\\\hline
  \Sexpr{paste0(colnames(defenceHyp$cspProfile)[6:10], collapse=" & ")}\\
