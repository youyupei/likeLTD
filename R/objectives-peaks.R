	...
	result.function <- function(scale,gradientS,gradientAdjust,interceptAdjust,
		...
		if(diagnose==TRUE)
			...
			repRes <- peaks.probabilities(hypothesis=hypothesis, cons=cons, 
							...
							repAdjust=repAdjust,
							detectionThresh=detectionThresh,
							dropin=dropin,dropoutWeight=dropoutWeight,
...
peaks.probabilities = function(hypothesis,cons,DNAcont,scale,
			...
			repAdjust,detectionThresh,dropin=NULL,
			dropoutWeight,doR=FALSE,diagnose=FALSE)
	{
...
peak.heights.per.locus = function(genotypeArray,alleles,heights,DNAcont,
			...
			detectionThresh,dropin=FALSE,dropoutWeight,diagnose=FALSE)
	{
	...
	Probs = apply(genotypeArray,MARGIN=2,FUN=function(x) probability.peaks(genotype=x,
	        ...
	        dropoutWeight=dropoutWeight,diagnose=diagnose))
	# diagnose
	if(diagnose==TRUE) return(Probs)
...
probability.peaks = function(genotype,alleles,heights,DNAcont,
		gradientS,meanD=NULL,meanO=NULL,interceptS,scale,
		degradation,fragLengths,fragProbs=NULL,LUSvals,repAdjust=NULL,
		detectionThresh,dropin=NULL,dropoutWeight,diagnose=FALSE)
	{
	...
	# function to return downweighted observed peak probability
	downWeightObserved = function(peakHeight,shape,scale,threshold,weight)
		{
		cdf = pgamma(q=threshold,shape=shape,scale=scale)
		pmfApprox = pgamma(q=peakHeight+0.5,shape=shape,scale=scale)-pgamma(q=peakHeight-0.5,shape=shape,scale=scale)
		out = pmfApprox*(1-(weight*(cdf/(1-cdf))))
		}
	if(length(dropoutIndex)>0)
		{
		if(length(dropoutIndex)!=length(peakHeights))
		    ...
		    pdf[-dropoutIndex] = mapply(FUN=function(x,k,t) downWeightObserved(x,k,t,threshold,dropoutWeight), 
		            x=peakHeights[-dropoutIndex], 
		            k=shapesVec[-dropoutIndex], 
		            t=scalesVec[-dropoutIndex])       
		    }
		# dropout = integral from 0 to threshold
		# weighted to increase weight of dropout
		pdf[dropoutIndex] = mapply(FUN=function(k,t) pgamma(q=detectionThresh,shape=k,scale=t)*(1+dropoutWeight), 
		        k=shapesVec[dropoutIndex], 
		        t=scalesVec[dropoutIndex])
		} else {
		# non-dropout = gamma point density
		    pdf = mapply(FUN=function(x,k,t) downWeightObserved(x,k,t,threshold,dropoutWeight), 
		            x=peakHeights, 
		            k=shapesVec, 
		            t=scalesVec)   
		}
