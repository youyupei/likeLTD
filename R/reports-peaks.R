...
minorAsDropin = function(csp,Qheights,unattributable,nU,dropinThresh=3)
	{
	if(nU==0) return(NULL)
	unattribHeights = sapply(1:length(csp$alleles),FUN=function(x) sapply(rownames(csp$alleles[[x]]),FUN=function(y) likeLTD:::getHeightsUnattrib(csp$heights[[x]][y,],csp$alleles[[x]][y,],unattributable[[x]][[y]])),simplify=FALSE)
