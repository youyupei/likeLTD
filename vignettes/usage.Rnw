...
common minimal set of input parameters, \texttt{defense.hypothesis} for the
...
defense hypothesis the queried individual is replaced by an unprofiled
...
convenience method provided by \likeLTD\, \texttt{optimization.params}. It
returns a list of adequate arguments for \texttt{optim} given a hypothesis:

<<optim, eval=FALSE, tidy=FALSE>>=
  params = optimization.params(defenseHyp, verbose=FALSE)
...
\begin{tabular}{l|ccc}
  ...
  rcont & \Sexpr{paste0(format(c(1, arguments$rcont), digits=3), collapse="&")} \\
...
\begin{tabular}{ccccc}
  ...
  \Sexpr{paste0(format(arguments$localAdjustment[1:5], digits=3), collapse=" & ")}\\\hline
  \Sexpr{paste0(colnames(defenseHyp$cspProfile)[6:10], collapse=" & ")}\\
  \Sexpr{paste0(format(arguments$localAdjustment[6:10], digits=3), collapse=" & ")}\\\hline
\end{tabular}\\[2ex]
